...
class Pfstools < Formula
  ...
  def install
    ...
    system "./configure",
    "CPATH=#{Formula.factory('netpbm').opt_prefix}/include", "--with-moc=/usr/local/bin/moc", "CXXFLAGS=-m64", "--prefix=#{prefix}", "--mandir=#{man}" "CXXFLAGS=-m64", "--prefix=#{prefix}", "--mandir=#{man}"
