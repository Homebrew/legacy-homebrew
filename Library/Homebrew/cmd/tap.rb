module Homebrew extend self
  ...
  def install_tap user, repo
    ...
    # 'homebrew-repo'.
    stdout = %x{git clone https://github.com/#{repouser}/#{repo} #{tapd} 2>/dev/null}
    if $? == 0
      puts stdout
    else
      stdout = %x{git clone https://github.com/#{repouser}/homebrew-#{repo} #{tapd} 2>&1}
      if $? == 0
        repo = "homebrew-" + repo
        puts stdout
      else
        abort <<-EOS.undent
          Neither #{repouser}/#{repo} nor #{repouser}/homebrew-#{repo} worked.
