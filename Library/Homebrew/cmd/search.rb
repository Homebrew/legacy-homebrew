    ...
    if ARGV.include? '--macports'
      ...
      if query
        $found = search_results.length
         ...
         threads = []
          threads << Thread.new { puts_columns search_tap "adamv", "alt", rx }
          threads << Thread.new { puts_columns search_tap "josegonzalez", "php", rx }
          threads << Thread.new { puts_columns search_tap "Homebrew", "versions", rx }
          threads << Thread.new { puts_columns search_tap "Homebrew", "dupes", rx }
          threads << Thread.new { puts_columns search_tap "Homebrew", "games", rx }
