...
class Tap
  ...
  def git_head
    ...
    return unless Utils.git_available? && git?
    path.cd { `git rev-parse --verify -q HEAD 2>/dev/null`.chuzzle }
  end
  ...
  def git_short_head
    raise TapUnavailableError, name unless installed?
    return unless Utils.git_available? && git?
    path.cd { `git rev-parse --short=4 --verify -q HEAD 2>/dev/null`.chuzzle }
  end
  ...
  def git_last_commit
    raise TapUnavailableError, name unless installed?
    return unless Utils.git_available? && git?
    path.cd { `git show -s --format="%cr" HEAD 2>/dev/null`.chuzzle }
  end
  ...
  def git_last_commit_date
    raise TapUnavailableError, name unless installed?
    return unless Utils.git_available? && git?
    path.cd { `git show -s --format="%cd" --date=short HEAD 2>/dev/null`.chuzzle }
  end
