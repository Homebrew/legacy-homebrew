...
class CurlDownloadStrategy < AbstractDownloadStrategy
  ...
  def initialize name, package
    ...
    if name.to_s.empty? || name == '__UNKNOWN__'
      @tarball_path = Pathname.new("#{HOMEBREW_CACHE}/#{basename}")
    else
  ...
  def stage
    ...
    when :xz
      ...
      chdir
    when :pkg
      safe_system '/usr/sbin/pkgutil', '--expand', @tarball_path, basename
      chdir
    when :rar
    ...
    when :p7zip
      ...
      FileUtils.cp @tarball_path, basename
    end
  end
...
class GzipOnlyDownloadStrategy < CurlDownloadStrategy
  def stage
    ...
    with_system_path { safe_system 'gunzip', '-f', basename }
  end
end
