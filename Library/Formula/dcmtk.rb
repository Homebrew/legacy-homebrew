class Dcmtk < Formula
  ...
  def install
    ...
    args << "-DWITH_OPENSSLINC=#{Formula["openssl"].opt_prefix}" if build.with? "openssl"
    args << "-DDCMTK_WITH_ICONV=YES" if build.with? "libiconv"
    args << "-DLIBICONV_DIR=#{Formula["libiconv"].opt_prefix}" if build.with? "libiconv"
    args << ".."
