...
class ModFastcgi < Formula
  ...
  def install
    ...
    if MacOS.xcode_version < "4.3"
      target_sdk = "#{MacOS.xcode_prefix}/SDKs/MacOSX#{MACOS_VERSION}.sdk"
    else
      target_sdk = "#{MacOS.xcode_prefix}/Platforms/MacOSX.platform/Developer/SDKs/MacOSX#{MACOS_VERSION}.sdk"
    end
    system "cp Makefile.AP2 Makefile"
    ENV.append_to_cflags "-mmacosx-version-min=#{MACOS_VERSION}"
    ENV.append_to_cflags "-isysroot #{target_sdk}"
