...
class Geocouch < Formula
  ...
  #  Leverage generic couchdb.rb formula for couchdb (and therefore geocouch) dependencies.
  depends_on 'couchdb'

  #  GeoCouch currently supports couch_version(s) 1.1.x and 1.2.x (other versions at your own risk).
  #  This formula supports GeoCouch 1.2.0 on top of Apache couchdb 1.2.0.
  def install
    #  Grab couchdb 1.2.x.
    ...
    #  Normal recipe "should" read:  (share/'couchdb/www/script/test/').install test_files
    #  which would symlink geocouch tests into the couchdb share.
    #  But couchdb seems to sandbox its web-readable files to the share/couchdb/www branch,
    #  and symlinks outside of that folder seem to violate couchdb's requirements.
    #  Consequently, we have to install geocouch tests directly inside the share/couchdb/www branch
    #  and not symlink them from the geocouch share branch (i.e., outside the couchdb sandbox).
    #  So for clarity sake, install/partition all the geocouch tests together into a tidy subfolder,
    #  and symlink them into place in the normal couchdb tests folder.
    ...
    #  Complete the install by referencing the geocouch tests in couch_tests.js (which runs the tests).
  ...
  def test
    puts <<-EOS.undent
      To test geocouch, start `couchdb` (with appropriate geocouch ERL_FLAGS) in a terminal and then:
  ...
  def caveats; <<-EOS.undent
    FYI:  geocouch installs as an extension of couchdb, so couchdb effectively becomes geocouch.
    However, you can use couchdb normally (using geocouch extensions optionally).
    NB:  one exception:  the couchdb test suite now includes several geocouch tests.
    
    To start geocouch manually and verify any geocouch version information (-V),

      ERL_FLAGS="-pa #{geocouch_share}/build"  couchdb -V

    For general convenience, export your ERL_FLAGS (erlang flags, above) in your login shell, and then start geocouch:
    ...
    Then start, check status of, and stop geocouch with the following three commands.
    ...
    and restart your couchdb.  (To see the uninstall instructions again, just run 'brew install geocouch'.)
