...
class Exodriver <Formula
  ...
  def install
    ...
    if ARGV.include? '--universal'
      system "sed 's/^CFLAGS += -fPIC -g -Wall$/#CFLAGS += -fPIC -g -Wall/' <Makefile.MacOSX >temp"
      system "sed 's/^#CFLAGS += -fPIC -g -Wall -arch i386 -arch x86_64$/CFLAGS += -fPIC -g -Wall -arch i386 -arch x86_64/' <temp >Makefile.MacOSX"
    end
