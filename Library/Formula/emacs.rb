...
class Emacs < Formula
  ...
  def caveats
    ...
    s += <<-EOS.undent
      ...
      install exuberant-ctags via brew with `brew install ctags` or `brew
      install ctags-exuberant` (they're the same package). (exuberant-ctags
      can provide both vim-style and emacs-style tags.)
    EOS
