...
class Ffmpeg < Formula
  ...
  def install
    ...
    if ARGV.include? "--tools"
      system "make alltools"
      tools = %w(cws2fws graph2dot lavfi-showfiltfmts pktdumper probetest qt-faststart trasher)
      bin.install tools.map { |tool| 'tools/' + tool }
    end
  end
