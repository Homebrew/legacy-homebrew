...
class CrosstoolNg < Formula
  ...
  def patches 
    ...
    p = [DATA]
    # This patch is already upstream.  The patch can be remove once a new crosstool-ng is release.
    ...
    # This patch is already upstream, but needed to removed a bash4ism that is preventing to work
    # correctly on version 1.17.0.  The patch can be remove once a new crosstool-ng is release.
  ...
  def install
    system "./configure", "--prefix=#{prefix}",
                          ...
                          "CFLAGS=-std=gnu89"
    #not parallel-safe, split in two lines
    system "make"
    system "make install"
  end
