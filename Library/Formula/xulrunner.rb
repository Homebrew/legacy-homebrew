...
class Xulrunner < Formula
  ...
  head do
    url ['http://ftp.mozilla.org/pub/mozilla.org/firefox/bundles/mozilla-central.hg','https://hg.mozilla.org/mozilla-central/'].join('|'), :using => HgBundleDownloadStrategy
  ...
  def install
    ...
    frameworks.mkdir
    system 'tar', '-xvjf', "objdir/dist/xulrunner-#{version.to_s[/\d+.\d/]}.en-US.mac64.tar.bz2", '-C', frameworks unless build.head?
