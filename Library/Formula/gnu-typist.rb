require 'formula'

class GnuTypist < Formula
  homepage 'http://www.gnu.org/software/gtypist/'
  url 'http://ftpmirror.gnu.org/gtypist/gtypist-2.9.1.tar.gz'
  mirror 'http://ftp.gnu.org/gnu/gtypist/gtypist-2.9.1.tar.gz'
  sha256 'a5885654aab74027999a67a9bbd7c3b6823479f89a6f1439244bf9c5536fb67d'

  depends_on 'gettext'

  PREFIX = `brew --prefix`.strip
  NCURSESW_PATH = "#{PREFIX}/include/ncursesw"

  def install
    # libiconv is not linked properly without this
    ENV.append 'LDFLAGS', '-liconv'

    # ncursesw functionality is now built into ncurses, but
    # gtypist still expects old filenames.
    unless File.exists?(NCURSESW_PATH) and File.directory?(NCURSESW_PATH)
      system "ln -s", "#{PREFIX}/include/ncurses", "#{NCURSESW_PATH}"
    end

    system "./configure", "--disable-dependency-tracking",
                          "--prefix=#{PREFIX}"
    system "make"
    system "make install"
  end
end
