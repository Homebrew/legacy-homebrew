...
class Ntfs3gNtfsprogs < Formula
  ...
  def install
    ...
    inreplace "ntfsprogs/Makefile.in" do |s|
      s.gsub! "/sbin", "$(sbindir)"
    end
    ...
    # Workaround for mkfs.ntfs needing to be installed in $(sbindir)
    system "mkdir -p /usr/local/sbin"
