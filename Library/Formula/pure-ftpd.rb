...
class PureFtpd < Formula
  ...
  def install
    args = [
      "--prefix=#{prefix}", "--mandir=#{man}", "--sysconfdir=#{etc}", "--disable-dependency-tracking",
      "--with-pam", "--with-altlog", "--with-puredb", "--with-throttling", "--with-ratios", "--with-quotas", "--with-ftpwho",
      "--with-virtualhosts", "--with-virtualchroot", "--with-diraliases", "--with-peruserlimits", "--with-tls", "--with-bonjour"
    ]
