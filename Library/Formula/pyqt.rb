...
class Pyqt < Formula
  ...
  def patches
    # On Mavericks we want to target libc++, but this requires a user specified qmake makespec.
    # Unfortunately user specified makespecs are broken in the configure.py script, so we have to
    # fix the makespec path handling logic.
    DATA
  end
  ...
  def install
    python do
      ...
      # We need to run "configure.py" so that pyqtconfig.py is generated, which is
      # needed by PyQWT for determining the correct build settings. But do the
      #  actual compile, we use the newer configure-ng.py.
