...
class Jbigkit < Formula
  ...
  def install
    # jbigkit includes libjbig, a C Library for creating JBIG images, and two CLI programs
    ...
    # This library only builds as static 64-bit correctly.
    # It fails with a version mismatch if you attempt to cross-compile with
    #      -arch x86_64 -arch i386
    # as well as failing if compiled for 32bit only, using by itself
    #      -arch i386
    # Clang was successful building for 64bit only, by removing the unrecognized CCFLAGS.
  ...
  def test
    puts
    system "jbgtopbm #{prefix}/examples/ccitt7.jbg | pbmtojbg - /tmp/testoutput.jbg"
    system "cmp #{prefix}/examples/ccitt7.jbg /tmp/testoutput.jbg"
    puts
