...
module ServicesCli
  class << self
    ...
    def running; %x{#{launchctl} list | grep homebrew}.chomp.split("\n").map { |svc| $1 if svc =~ /(homebrew.+)\z/ }.compact end
    ...
    def cleanup
      ...
      Dir[path + 'homebrew*.plist'].each do |file|
...
class Service
  ...
  def self.from(path_or_label)
    return nil unless path_or_label =~ /homebrew[^\.]*\.[^\.]+\.([^\.]+)(\.plist)?\z/
  ...
  # Path to a static plist file, e.g `homebrew.mxcl.<formula>.plist`.
  ...
  def generate_plist(data = nil)
    ...
    # replace "template" variables and ensure label is always, always <prefix>.<formula>
